...
public class SquarerootPiece extends GamePiece {
    ...
    public void render(Graphics2D graphics2D, int xPos, int yPos, int width, int height) {
        graphics2D.drawImage(ResourceManager.INSTANCE.findResourceFor(FilteredImageResource.class, SQUAREROOT_PIECE_RES_NAME + getColour().name()).getImageWrapper().getImage(), xPos, yPos, width, height, null);
    }
}
